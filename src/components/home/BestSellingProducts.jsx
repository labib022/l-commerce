...
const BestSellingProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await api.get('/products?limit=8&sort=-ratings');
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => scroll(-1)} className="w-8 h-8 bg-white border border-gray-200 rounded-full flex items-center justify-center hover:bg-gray-50 transition-colors shadow-sm">‹</button>
            <button onClick={() => scroll(1)} className="w-8 h-8 bg-white border border-gray-200 rounded-full flex items-center justify-center hover:bg-gray-50 transition-colors shadow-sm">›</button>
          </div>
        </div>
      </div>
